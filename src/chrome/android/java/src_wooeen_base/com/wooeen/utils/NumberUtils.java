...
public class NumberUtils {
    ...
    public static final NumberFormat NUMERO = new DecimalFormat("##0.00; -##0.00",
            DecimalFormatSymbols.getInstance(Locale.US));
    public static final NumberFormat REAL = new DecimalFormat("'$' ##0.00;'$' -##0.00",
            DecimalFormatSymbols.getInstance(Locale.US));
    public static final NumberFormat REAL_INTEIRO = new DecimalFormat("'$' ##0;'$' -##0",
            DecimalFormatSymbols.getInstance(Locale.US));
    public static final NumberFormat MONETARY = new DecimalFormat("#,##0.00;-#,##0.00",
            new DecimalFormatSymbols(Locale.US));
    public static final NumberFormat PORCENTAGEM_INT = new DecimalFormat("#'%';-#'%'",
            DecimalFormatSymbols.getInstance(Locale.US));
    public static final NumberFormat PORCENTAGEM_PRINT = new DecimalFormat("###.##'%';-###.##'%'",
            DecimalFormatSymbols.getInstance(Locale.US));
    public static final NumberFormat PORCENTAGEM = new DecimalFormat("##0.00'%';-##0.00'%'",
            DecimalFormatSymbols.getInstance(Locale.US));
    public static final NumberFormat INTEIRO = new DecimalFormat("##0;-##0",
            DecimalFormatSymbols.getInstance(Locale.US));
    public static final NumberFormat INTEIRO_MASK = new DecimalFormat("#,##0;-#,##0",
            DecimalFormatSymbols.getInstance(Locale.US));
    public static final NumberFormat INTEIRO_NO_MASK = new DecimalFormat("###0;-###0",
            DecimalFormatSymbols.getInstance(Locale.US));
    ...
    public static String percentIntToString(double valor) {
        return PORCENTAGEM_INT.format(valor);
    }
...
//			double numero = number.doubleValue();
    ...
    public static double intToDouble(int number) {
		...
		if (number < 10) {
			...
			value = "" + number;
			value = value.substring(0, value.length() - 2) + "." + value.substring(
					value.length() - 2);
		}
    ...
    public static String realToString(String currency, String country, String lg, double valor) {
        if(TextUtils.isEmpty(country) || TextUtils.isEmpty(lg))
		...
		if (!TextUtils.isEmpty(country) && !TextUtils.isEmpty(lg)) {
			format = NumberFormat.getCurrencyInstance(new Locale(lg, country));
		} else {
