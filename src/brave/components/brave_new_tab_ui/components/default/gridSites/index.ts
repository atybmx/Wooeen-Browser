...
export const List = styled('div')<ListProps>`
  ...
  height: 100%;
  display: grid;
  grid-template-columns: repeat(${p => Math.min(p.blockNumber, 6).toString()}, 110px);
  justify-content: var(--ntp-item-justify, start);
  padding: 24px 0 24px 24px;

  @media screen and (max-width: 700px) {
    grid-template-columns: repeat(${p => Math.min(p.blockNumber, 3).toString()}, 110px);
  }

  @media screen and (max-width: 390px) {
    grid-template-columns: repeat(${p => Math.min(p.blockNumber, 2).toString()}, 110px);
  }
`
