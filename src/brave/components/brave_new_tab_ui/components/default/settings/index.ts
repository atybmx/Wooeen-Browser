...
export const SettingsSidebarActiveButtonSlider =
  ...
  height: 48px;
  width: 4px;
  background: linear-gradient(93.83deg, ${p => p.theme.color.brandBrave} -3.53%, ${p => p.theme.color.brandBrave} 110.11%);
...
export const SettingsSidebarButtonText = styled('span')<{ isActive: boolean }>`
  ...
  &:after {
    ...
    background-image: linear-gradient(93.83deg, ${p => p.theme.color.brandBrave} -3.53%, ${p => p.theme.color.brandBrave} 110.11%);
    overflow: hidden;
    text-overflow: ellipsis;
    transition: opacity var(--sidebar-button-transition-timing) ease-in-out,
                font-weight var(--sidebar-button-transition-timing) ease-in-out;
