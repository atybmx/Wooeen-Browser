...
export const defaultState: NewTab.State = {
  ...
  widgetStackOrder: ['balance', 'cashback', 'recommendation'],
  binanceState: {
    userTLD: 'com',
    userLocale: 'en',
    initialFiat: 'USD',
  ...
  woeTasks: undefined,
  woeOffers: undefined,
  woeCoupons: undefined,
  woeCountry: undefined
}
...
const getMigratedWidgetOrder = (state: NewTab.State) => {
  ...
  if(showBalance)
    widgetStackOrder.push('balance');
  if(showCashback)
    widgetStackOrder.push('cashback');
  if(showRecommendation)
    widgetStackOrder.push('recommendation');
...
export const debouncedSave = debounce<NewTab.State>((data: NewTab.State) => {
  if (data) {
    ...
    const dataToSave = {
      ...
      woeUser: data.woeUser,
      woePosts: data.woePosts,
      woeTasks: data.woeTasks,
      woeOffers: data.woeOffers,
      woeCoupons: data.woeCoupons,
      woeCountry: data.woeCountry
    }
