...
let memoryTodayData: Wooeen.FeedTask | undefined
let readLock: Promise<void> | null
let isKnownRemoteUpdateAvailable = false

const STORAGE_KEY = 'woetsk'
const STORAGE_SCHEMA_VERSION = 1
const STORAGE_KEY_LAST_REMOTE_CHECK = 'woetskLastRemoteUpdateCheck'

type FeedInStorage = {
  storageSchemaVersion: number,
  feed: Wooeen.FeedTask
}
...
async function setStorageData (feed: Wooeen.FeedTask) {
  ...
  readLock = new Promise(async function (resolve, reject) {
    try {
        ...
        .then(
          ...
          (error) => {
            console.error('WOETSK: Could not get feed contents');
          }
        )
      ...
      console.error('WOETSK: Could not process feed contents')
      reject(e)
    } finally {
      readLock = null
    }
...
export function checkout(endpoint: string) : Wooeen.Task | undefined {
  if(!endpoint) return;
