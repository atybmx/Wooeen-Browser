...
void BraveRenderViewContextMenu::InitMenu() {
  ...
  //WOE disable incognito option
  int indexinc = menu_model_.GetIndexOfCommandId(IDC_CONTENT_CONTEXT_OPENLINKOFFTHERECORD);
  if (indexinc != -1)
    menu_model_.RemoveItemAt(indexinc);
