...
//    BraveVPN.fetchLastUsedRegionDetail()
    ...
    print("WOE initiandoooo")
    //WOE init DB
    WooeenDAO.initDB()
    
    //WOE sync data schedule
    woeRegisterBackgroundTasks()
  ...
  // MARK: WOE Register Background Tasks
  private func woeRegisterBackgroundTasks() {
    BGTaskScheduler.shared.register(forTaskWithIdentifier: "com.wooeen.data_sync", using: nil) {[weak self] task in
      print("WOE register")
      self?.woeHandleSyncDataTask(task: task as! BGProcessingTask)
    }
  ...
  func woeHandleSyncDataTask(task: BGProcessingTask) {
    print("WOE handlee")
    //Reschedule the task
    woeScheduleSyncData()
  ...
  func woeScheduleSyncData() {
    print("WOE scheduleSyncData")
    let request = BGProcessingTaskRequest(identifier: "com.wooeen.data_sync")
    // Need to be true if your task needs network processing. Defaults to false.
    ...
    do {
      ...
      print("WOE Could not schedule sync data fetch: \(error)")
    }
  }
