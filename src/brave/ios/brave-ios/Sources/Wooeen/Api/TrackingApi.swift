...
public class TrackingApi{
    ...
    public func search(pg: Int?, qtdPerPage: Int?, callback: @escaping (Result<[TrackingTO], Error>)->Void){
        if(country ?? "").isEmpty{
            callback(.failure(WooeenApiError.requiredParams))
            return
        }

        var urlBuilder = URLComponents(string: "\(WooeenApi.endpoint)/tracking/get")
        ...
        let task = URLSession.shared.dataTask(with: request) { (data, response, error) in
            ...
            do {
                let resultData = try JSONDecoder().decode(TrackingResponse.self, from: data)
                if(resultData.result){
                    if let trackings = resultData.callback{
                        callback(.success(trackings))
                    }
                }else{
