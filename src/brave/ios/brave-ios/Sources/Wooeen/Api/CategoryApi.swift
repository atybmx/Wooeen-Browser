...
public class CategoryApi{
    ...
    public func search(pg: Int?, qtdPerPage: Int?, callback: @escaping (Result<[CategoryTO], Error>)->Void){
        if(country ?? "").isEmpty{
            callback(.failure(WooeenApiError.requiredParams))
            return
        }

        var urlBuilder = URLComponents(string: "\(WooeenApi.endpoint)/category/get")
        ...
        let task = URLSession.shared.dataTask(with: request) { (data, response, error) in
            ...
            do {
                let resultData = try JSONDecoder().decode(CategoryResponse.self, from: data)
                if(resultData.result){
                    if let categories = resultData.callback{
