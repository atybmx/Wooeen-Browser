...
class TopToolbarView: UIView, ToolbarProtocol {
  ...
  lazy var actionButtons: [UIButton] = [
    ...
    shieldsButton, rewardsButton, woecashbackButton
  ].compactMap { $0 }
  ...
  private(set) lazy var shieldsButton: ToolbarButton = {
    ...
    private(set) lazy var woecashbackButton: ToolbarButton = {
      let button = ToolbarButton()
      button.setImage(UIImage(sharedNamed: "wooeen.logo"), for: .normal)
      button.addTarget(self, action: #selector(didTapBraveShieldsButton), for: .touchUpInside)
      button.imageView?.contentMode = .scaleAspectFit
      button.accessibilityLabel = Strings.bravePanel
      button.imageView?.adjustsImageSizeForAccessibilityContentSizeCategory = true
      button.accessibilityIdentifier = "urlBar-woecashbackButton"
      button.contentEdgeInsets = .init(top: 4, left: 5, bottom: 4, right: 5)
      return button
    }()
  ...
  init(voiceSearchSupported: Bool, privateBrowsingManager: PrivateBrowsingManager) {
    ...
    //WOE removendo shields e rewards, adicionando cashback
    //shieldsRewardsStack.addArrangedSubview(shieldsButton)
    //shieldsRewardsStack.addArrangedSubview(rewardsButton)
    shieldsRewardsStack.addArrangedSubview(woecashbackButton)
  ...
  private func updateForTraitCollection() {
    ...
    woecashbackButton.snp.remakeConstraints {
      $0.size.equalTo(pointSize)
    }
  ...
  func refreshShieldsStatus() {
    ...
    /// Update the cashback icon based on whether or not cashback are enabled for this site
    func refreshWoecashbackStatus(){
        let cashbackOnIcon = "wooeen.ico"
        let cashbackOffIcon = "wooeen_off.ico"
        let cashbackDonationIcon = "wooeen_donation.ico"
        var cashbackIcon = cashbackOffIcon
        if let currentURL = currentURL{
            if !currentURL.isLocal && !currentURL.isLocalUtility {
                if let domainRoot = currentURL.baseDomain{
                    if let trk = TrackingUtils.tracked(domain: domainRoot){
                        if trk.advertiserType == 2 {
                            cashbackIcon = cashbackDonationIcon
                        }else{
                            cashbackIcon = cashbackOnIcon
                        }
                    }
                }//root domain
            }//end local
        ...
        woecashbackButton.setImage(UIImage(sharedNamed: cashbackIcon), for: .normal)
    }
