...
extension BrowserViewController: WKNavigationDelegate {
  ...
  private static var mWooeenTrackingHandler:TrackingHandler = TrackingHandler()
  ...
  public func webView(_ webView: WKWebView, didStartProvisionalNavigation navigation: WKNavigation!)
  {
    ...
    if let url = webView.url{
        if !url.isLocal && !url.isLocalUtility {
            let mUrlNavig = url.absoluteString
            
            if let host = url.host() {
                let domain = host.replacingOccurrences(of: "^www.", with: "", options: .regularExpression)
                
                // verify login
                if domain == "app.wooeen.com"{
                    domainWooeen()
                }//end domain
                
                //insert country parameter in search
                if let cr = UserDAO.getCountry(){
                    if domain == "search.wooeen.com" && !mUrlNavig.contains("cr="){
                        var goTo = mUrlNavig
                        if goTo.contains("?"){
                            goTo += "&"
                        }else{
                        ...
                        tabManager.selectedTab?.loadRequest(URLRequest(url: URL.init(string: goTo)!))
                    }//end search without cr
                }//end getcountry
           
            }//end host
            
            if let user = UserDAO.getUser(){
                if let domainRoot = url.baseDomain{
                    if let tabId = tabManager.selectedTab?.id{
                        domainTrack(tabId: tabId, domain: domainRoot, mUrlNavig: mUrlNavig, user: user)
                    }
                }//end domainRoot
            }//end getUser
        }//end local
    ...
    fileprivate func domainTrack(tabId:UUID, domain:String, mUrlNavig:String, user: UserTO){
        var domainHandler = BrowserViewController.mWooeenTrackingHandler.getDomain(tabId: tabId)
        if BrowserViewController.mWooeenTrackingHandler.isTracking(tabId: tabId){
            if domain == domainHandler{
                BrowserViewController.mWooeenTrackingHandler.tracked(tabId: tabId)
            }
        }else{
            if domainHandler != nil && domain != domainHandler{
                BrowserViewController.mWooeenTrackingHandler.changeDomain(tabId: tabId)
                domainHandler = nil
            }
            
            if domainHandler == nil{
                //add pretracking already tracked with social
                if mUrlNavig.contains("=\(user.id)_"){
                    BrowserViewController.mWooeenTrackingHandler.addPreTracking(tabId: tabId)
                }
                
                if let tracking = TrackingUtils.tracked(domain: domain){
                    //get script data from advertiser dao
                    var advertiser:AdvertiserTO?
                    if let advertiserId = tracking.advertiserId{
                        var advertiserTOP = AdvertiserTOP()
                        advertiserTOP.id = advertiserId
                        if let advertisers = AdvertiserDAO().search(search: advertiserTOP), !advertisers.isEmpty{
                            advertiser = advertisers[0]
                        }
                    }
                    if advertiser == nil{
                        advertiser = AdvertiserTO(id: tracking.advertiserId ?? 0)
                    }
                    
                    BrowserViewController.mWooeenTrackingHandler.addTracking(
                        tabId: tabId,
                        domain: domain,
                        id: tracking.id,
                        platform: tracking.platformId,
                        advertiser: advertiser?.id,
                        checkoutEndpoint: advertiser?.checkout?.endpoint,
                        checkoutData: advertiser?.checkout?.data,
                        productEndpoint: advertiser?.product?.endpoint,
                        productData: advertiser?.product?.data,
                        queryEndpoint: advertiser?.query?.endpoint,
                        queryData: advertiser?.query?.data)
                    
                    if BrowserViewController.mWooeenTrackingHandler.isTracking(tabId: tabId){
                        if let goTo = TrackingUtils.parseTrackingLink(
                            deeplink: tracking.deeplink,
                            params: tracking.params,
                            link: mUrlNavig,
                            userId: user.id,
                            source: TrackingUtils.sourceApp,
                            affiliate: 0){
                            tabManager.selectedTab?.loadRequest(URLRequest(url: URL.init(string: goTo)!))
                        }
                    }
  ...
  fileprivate func domainWooeen(){
      WKWebsiteDataStore.default().httpCookieStore.getAllCookies { cookies in
      
          //read the cookies
          let cookies = cookies.filter {
              TrackingUtils.woeEligibleDomains.contains($0.domain)
          }
          
          var uti:String?, uta:String?, ui:Int?, un:String?
          ...
          //CRUD in user token
          var idToken:String?, accessToken:String?
          if let token = UserDAO.getUserToken(){
              idToken = token.idToken
              accessToken = token.accessToken
          }
          if let uti = uti, let uta = uta, !uti.isEmpty, !uta.isEmpty{
              if let idToken = idToken, let accessToken = accessToken, !idToken.isEmpty, !accessToken.isEmpty{
                  if uti != idToken || uta != accessToken{
                      if let ui = ui, let un = un, ui > 0, !un.isEmpty{
                          var user = UserTO(id: ui)
                          user.name = un
                          UserDAO.saveUser(user: user)
                      }
                      let token = UserTokenTO(idToken: uti, accessToken: uta)
                      UserDAO.saveUserToken(token: token)
                      UserDAO.removeUser()
                      WooeenSync.syncUser()
                  }
              }else{
