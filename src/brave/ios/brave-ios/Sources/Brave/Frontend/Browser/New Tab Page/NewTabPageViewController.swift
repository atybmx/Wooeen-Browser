...
class NewTabPageViewController: UIViewController {
    ...
    sections = [
      HeaderSectionProvider(
        actionHandler: { [weak self] in
          self?.handleHeaderAction($0)
        }
      ),
      LogoSectionProvider(action: { [weak self] in
        self?.delegate?.focusURLBar()
      }),
      SearchbarSectionProvider(action: { [weak self] in
        self?.delegate?.focusURLBar()
      }),
      BottomActionsProvider(
        openCouponsPressed: { [weak self] in
            var queryCR = ""
            if let cr = UserDAO.getCr(){
              queryCR += "?cr=\(cr)"
            }
            self?.delegate?.navigateToInput(
                "https://app.wooeen.com/c/coupons\(queryCR)",
                inNewTab: false,
                switchingToPrivateMode: false
              )
         },
        openDiscountsPressed: { [weak self] in
            var queryCR = ""
            if let cr = UserDAO.getCr(){
              queryCR += "?cr=\(cr)"
            }
            self?.delegate?.navigateToInput(
                "https://app.wooeen.com/c/timeline\(queryCR)",
                inNewTab: false,
                switchingToPrivateMode: false
              )
         },
        openEarnPressed: { [weak self] in
            self?.delegate?.navigateToInput(
                "https://app.wooeen.com/u/challenges",
                inNewTab: false,
                switchingToPrivateMode: false
              )
         },
        openAdvertisersPressed: { [weak self] in
            var queryCR = ""
            if let cr = UserDAO.getCr(){
              queryCR += "?cr=\(cr)"
            }
            self?.delegate?.navigateToInput(
                "https://app.wooeen.com/c/advertisers\(queryCR)",
                inNewTab: false,
                switchingToPrivateMode: false
              )
         },
        openMyDataPressed: { [weak self] in
            self?.delegate?.navigateToInput(
                "https://app.wooeen.com/u/dashboard",
                inNewTab: false,
                switchingToPrivateMode: false
              )
         },
       openSupportPressed: { [weak self] in
           let phone = "376350991"
           var message = ""
           if let user = UserDAO.getUser(){
               message += "\(user.country?.id ?? "")\nID: \(user.id)\n\(user.name ?? "")"
           }
  ...
  override func viewDidLoad() {
    ...
    view.backgroundColor = FrontUtils.rgbaToUIColor(red: 242, green: 243, blue: 245, alpha: 1)

    //collectionView.backgroundView = backgroundButtonsView

    //feedOverlayView.headerView WOE header do brave news
    ...
    //WOE disable background image
    //setupBackgroundImage()
    //WOE disable background video
  ...
  private func hideNotification() {
    ...
    private func handleHeaderAction(_ action: HeaderSectionProvider.Action){
        switch action {
        case .headerRightAction(.balanceButtonTapped):
            delegate?.navigateToInput(
              "https://app.wooeen.com/u/dashboard",
              inNewTab: false,
              switchingToPrivateMode: false
            )
        case .headerRightAction(.cashbackActiveButtonTapped):
            var queryCR = ""
            if let cr = UserDAO.getCr(){
                queryCR += "?cr=\(cr)"
            }
            delegate?.navigateToInput(
              "https://app.wooeen.com/u/login\(queryCR)",
              inNewTab: false,
              switchingToPrivateMode: false
            )
        default:
